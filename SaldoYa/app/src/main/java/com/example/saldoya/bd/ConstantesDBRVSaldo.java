package com.example.saldoya.bd;

public class ConstantesDBRVSaldo {
    public static final String NOMBRE_DB= "DBSaldos";
    public static final int VERSION_DB= 1;

    public static final String T_SALDOS = "saldos";
    public static final String T_SALDOS_ID = "id";
    public static final String T_SALDOS_IDTIENDA = "idTienda";
    public static final String T_SALDOS_CEDULA = "cedula";
    public static final String T_SALDOS_SALDO = "saldo";

}
